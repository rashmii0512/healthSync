...
app = Flask(__name__)
UPLOAD_FOLDER = os.path.join(os.getcwd(), 'uploads')  # Ensure this points to your desired uploads directory
...
def getXrayDescription():
    global description_list
    with open('Medical_dataset/xrayDescription.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        line_count = 0
        for row in csv_reader:
            _description = {row[0]: row[1]}
...
def get_bot_response():
    ...
    if session['step'] == "BFS":
        ...
        return "Can you precise your main symptom Mr/Ms " + session["name"] + " or upload an Xray?"
    if session['step'] == "FS":
        sym1 = s
...
# IMAGE_DIR = "C:/Users/rashm/majorProject/x-ray-code/nih/Images/"
...
base_model = DenseNet121(weights='C:/Users/rashm/majorProject/x-ray-code/nih/densenet.hdf5', include_top=False)
...
model.load_weights("C:/Users/rashm/majorProject/x-ray-code/nih/pretrained_model.h5")

df = pd.read_csv("C:/Users/rashm/majorProject/x-ray-code/train-small.csv")
...
def upload_xray():
    print("uploading img")
    session['step'] = 'xrayDetection'
    
    # Check if the request contains a file
    if 'xray-image' not in request.files:
        return 'No file part', 400

    file = request.files['xray-image']
    if file.filename == '':
    ...
    if file:
        ...
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], "img.jpg")
        
        # Check if the image already exists, and delete if it does
        if os.path.exists(filepath):
            os.remove(filepath)
            print(f"Existing image deleted: {filepath}")

        # Save the new file
        file.save(filepath)
        print(f"Image Directory: {image_dir}, Image Name: {fname}")

        # Run the X-ray detection model using the full file path
        disease_probabilities = ut5.predictions(model, filepath, app.config['UPLOAD_FOLDER'], df, labels, labels)
        sorted_diseases = sorted(disease_probabilities.items(), key=lambda item: item[1], reverse=True)
        
        # Get the top 3 diseases with probability > 70%
        top_3_diseases = [(disease, prob) for disease, prob in sorted_diseases[:3] if prob > 0.7]

        # Prepare the response
        response = "Top 3 predicted diseases with probability > 70%:<br><br>"
        
        if top_3_diseases:
            for disease, probability in top_3_diseases:
                response += f"<strong>{disease}</strong>: {probability:.4f} : "
                
                # Provide information from the description list or a Wikipedia link
                if disease in description_list:
                    response += description_list[disease] + "<br/>"
                else:
                    disease_name = disease.replace(" ", "_")
            response += "<strong><br>Your diagnosis was perfectly completed. <br>Do you need another medical consultation (yes or no)?</strong>"
        
        else:
            response = "No diseases with probability greater than 70%. Your diagnosis is completed. Do you need another medical consultation (yes or no)? "
    
        session['step'] = "BYE"
        return response


    return "Please upload the image through the correct step."
